<template>
  <b-container>
    <b-row align-h="center">
      <div class="col-md-4">
        <img class="poster" :src="`${$store.getters.getServerUrl}`+movie_data.poster" alt="poster">
      </div>
      <div class="col-md-8">
        <div class="card-body">
          ...
          <p class="card-text">{{ movie_data.description }}</p>
          <hr class="featurette-divider">
          <p class="fw-bolder">Мнение автора</p>
          <p class="card-text">{{ movie_data.opinion }}</p>
          <hr class="featurette-divider">
          <p class="fw-bolder">Оценка</p>
          <p class="card-text">{{ getMarkStatus(movie_data.mark) }} -- {{ getMarkValue(movie_data.mark) }}</p>
...
export default {
  name: "InfoCard",
  props: ["movie_data", "name", "list_genre"],
  mounted() {
    this.loadListMarks()
  },
  methods: {
